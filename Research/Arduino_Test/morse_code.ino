...
const long DOT_DASH_THRESHOLD = 200; // if <200 = dot and if >20 = dash
const long WORD_SEP_THRESHOLD = 700; // if >700 = go to new line


// —-- variables —--
long pressStartTime = 0; //time
long pressDuration = 0; //time
int lastButtonState = HIGH; //button state (on or off)
...
void setup() {
 ...
 Serial.begin(9600);
 Serial.println("Morse Code Ready (No Slash)!");
 Serial.println("Timing: . < 200ms, - < 700ms, New Line > 700ms");
 Serial.println("---");
}
...
if (currentButtonState == LOW && lastButtonState == HIGH) {
   pressStartTime = millis(); // Record the current time
}
 ...
 //--- Releasing the Button ---
   ...
   // Checking for the >700 press (new line)
  if (pressDuration >= WORD_SEP_THRESHOLD) {
     Serial.println(); // Go to the next line
  }
   // Checking for [(> or < 200) and (< 700)] (dot or dash)
  else if (pressDuration >= DOT_DASH_THRESHOLD) {
    Serial.print("-");
  }
   // Checking for < 200 specifically (dot)
  else {
    Serial.print(".");
  }
 }
 ...
 // Preserving the current state as now the last for next loop iteration
